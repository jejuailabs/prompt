...
export function buildLtx2bWorkflow(input: VideoRenderInput): Record<string, unknown> {
  const [width, height] = input.comparison ? [832, 480] : input.aspectRatio === '9:16' ? [512, 768] : input.aspectRatio === '1:1' ? [640, 640] : [768, 512];
  ...
  return {
    ...
    '102': { class_type: 'RandomNoise', inputs: { noise_seed: input.seed ?? Math.floor(Math.random() * 2147483647) } },
