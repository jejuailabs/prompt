...
export default function AiToolsView() {
  ...
  const seoSlug: Record<string, string> = { 'tool-shortform': 'shortform-video', 'tool-3d': '3d-asset', 'tool-detailpage': 'detail-page', 'tool-game': 'game-maker' };
  return <div className="mx-auto w-full max-w-7xl p-4 md:p-6 lg:p-8">
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      {items.map((tool) => <Card key={tool.id} className="group p-5 transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-lg">
        ...
        <Button className="mt-5 w-full" disabled={tool.status === 'preparing'} onClick={() => { const slug = seoSlug[tool.id]; if (slug) router.push(`/tools/${slug}`); else navigate('pipeline-run', { id: tool.id.replace('tool-', 'pipeline-') }); }}><Sparkles className="size-4" />{tool.status === 'active' ? '도구 열기' : '준비 중'}</Button>
      </Card>)}
    </div>
  </div>;
}
