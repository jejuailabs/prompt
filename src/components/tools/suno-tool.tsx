...
export function SunoTool() {
  ...
  const generate = async () => {
    ...
    for (let i = 0; i < count; i++) {
      try {
        const res = await fetch("/api/tools/suno", {
          method: "POST",
  ...
  const analyzeLyrics = async () => {
    ...
    try {
      const res = await fetch("/api/tools/lyrics-analyze", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const generateLyrics = async () => {
    ...
    try {
      const res = await fetch("/api/tools/lyrics-gen", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const generateCover = async () => {
    setGenCover(true);
    try {
      const res = await fetch("/api/tools/image", { method: "POST", headers: { "Content-Type": "application/json" },
