...
export function AceMusicTool() {
  ...
  return <section className="mt-10 rounded-2xl border bg-card p-6 shadow-sm">
    ...
    <div className="mt-5 grid gap-4 sm:grid-cols-2"><fieldset><legend className="mb-2 text-sm font-medium">길이</legend><div className="flex flex-wrap gap-2">{[30, 60, 90, 180, 240].map((value) => <button key={value} type="button" onClick={() => setDuration(value)} disabled={active} className={`rounded-md border px-4 py-2 text-sm ${duration === value ? 'border-primary bg-primary text-primary-foreground' : 'bg-background'}`}>{value >= 60 ? `${value / 60}분` : `${value}초`}</button>)}</div><p className="mt-2 text-xs text-muted-foreground">일반 곡 길이인 3분·4분까지 생성합니다. 길이에 비례해 처리 시간과 크레딧이 늘어납니다.</p></fieldset><label className="grid max-w-44 gap-2 text-sm font-medium">BPM <input inputMode="numeric" value={bpm} onChange={(event) => setBpm(event.target.value.replace(/\D/g, '').slice(0, 3))} placeholder="자동" className="rounded-lg border bg-background px-3 py-2 text-sm" /></label></div>
